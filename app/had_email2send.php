			...
			if(mysql_num_rows($resultfollow)){
				...
				while($row2 =  mysql_fetch_object($resultfollow)){
					...
					if($credit > 0){
						...
						$id = $placeId .'|'. $row2->id;
						$id = base64_encode( $id );
						$footer = '<p><strong>Powered by Tabluu & sent by:</strong><br/>'. $frow->businessName .'<br/>'.$frow->address .' ' . $frow->city .' '. $frow->country .'<br/>'. $frow->contactNo .'</p>'. '<p>To unfollow or unsubscribe, please click below link:<br/>http://camrally.com/unsubscribe.php?id='. $id .'<a/></p>';					
