			...
			?>
			 <div class="btnwrap">
				<div style="clear:both;text-align:right;">
					<div class="btn-take-isselfie1"><a style="text-decoration:none;color: #fff;" href="<?=$campaignsite?>" target="_blank"><?php echo ($btntxt == '' ? 'See the Rally!' : $btntxt)  ?></a></div>
					<?php
					if($connect->liteID != $row->productId){
						if($row->booknow){
		...
		<div id="topmenu">
			...
			<div class="MerchantWrapper">
				 <div class="MerchantHead">
					...
					echo '<a href="'.$campaignsite.'"  class="color-button" target="_blank"><span>' .($btntxt == '' ? 'See the Rally!' : $btntxt) . '</span></a>'; 
					echo '<div class="clear" style="padding:5px 0"></div>';
                    if($connect->liteID != $row->productId){
						if($booksite){
