...
switch($opt){
	...
	case 'ratesave':
				 ...
				 $userName = $_REQUEST['userName'];$userId = $_REQUEST['userId'];$photo_url =$_REQUEST['photo_url'];$id = $_REQUEST['placeId'];$date = date('Y-m-d H:i:s');$email = $_REQUEST['email'];$source = $_REQUEST['source'];$param = $_REQUEST['param'];
				...
				$query = mysql_query('INSERT INTO businessplace_'.$id.' SET userName="'.$userName.'",userId="'.$userId.'",photo_url="'.$photo_url.'",source="'.$source .'",date="'.$date.'",feedsource="'.$param.'",labelId="'.$label.'"') or die(mysql_error());
